...
export default function StudentPage() {
  ...
  const validAnnouncements = announcements.filter(a => {
    if (!a.expires_at) return true; // ไม่มีวันหมดอายุ → แสดง
    return new Date(a.expires_at) > now; // ยังไม่หมดอายุ
  });

  // แยก pinned / non-pinned จากที่ผ่านการกรองแล้ว
  const pinnedAnnouncements = validAnnouncements.filter(a => a.is_pinned);
  const regularAnnouncements = validAnnouncements.filter(a => !a.is_pinned);
