...
import { fetchCurriculumSummary } from "@/services/curriculum";
import { Loader2, Users, BookOpen, FileText, Calendar, ArrowUpRight } from "lucide-react";
  ...
  useEffect(() => {
    const checkAuthAndLoadData = async () => {
      ...
      try {
        ...
        setUserName(user.first_name_th || user.first_name_en || "Admin");

        setLoading(true);
        /**
         * Fetch dashboard data:
         * 1. Users count
         * 2. Curriculum summary (for total count)
         * 3. Admin announcements
         */
        const [usersRes, curriculumRes, announcementsRes] = await Promise.all([
          fetchAllUsers(),
          fetchCurriculumSummary(),
          AnnounceService.getAdminAnnouncements(),
        ]);
  ...
  return (
    <div className="p-6 md:p-10 max-w-8xl mx-auto space-y-8 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-white p-8 rounded-3xl shadow-lg shadow-orange-500/5 border border-orange-100/50 hover:shadow-2xl hover:shadow-orange-500/10 hover:border-orange-200 transition-all duration-300 relative overflow-hidden h-full">
            <div className="flex items-start justify-between relative z-10">
              <div className="space-y-4">
                ...
                <h2 className="text-6xl font-black text-gray-800 tracking-tighter group-hover:text-orange-500 transition-colors duration-300">{userCount}</h2>
              ...
              <div className="absolute -right-6 -bottom-6 opacity-5 group-hover:opacity-10 transition-opacity duration-300 transform group-hover:scale-110 group-hover:-rotate-12">
                <Users className="w-40 h-40 text-orange-600" />
              </div>
        ...
        {/* Curricula Card */}
        <Link href="/admin/curricula" className="block group">
          <div className="bg-white p-8 rounded-3xl shadow-lg shadow-blue-500/5 border border-blue-100/50 hover:shadow-2xl hover:shadow-blue-500/10 hover:border-blue-200 transition-all duration-300 relative overflow-hidden h-full">
            <div className="flex items-start justify-between relative z-10">
              <div className="space-y-4">
                ...
                <h2 className="text-6xl font-black text-gray-800 tracking-tighter group-hover:text-blue-500 transition-colors duration-300">{curriculumCount}</h2>
      ...
      <div className="bg-white rounded-[2rem] p-8 md:p-10 shadow-xl shadow-gray-200/50 border border-gray-100">
        <div className="flex items-center justify-between mb-6">
          ...
          <Link href="/admin/announcements" className="text-sm font-semibold text-orange-500 hover:text-orange-600 hover:underline">
            จัดการข่าวสาร
          </Link>
        </div>
