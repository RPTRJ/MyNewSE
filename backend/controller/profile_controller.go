...
// UpdatePersonalInfo handles updating personal info for already-onboarded users
func (pc *ProfileController) UpdatePersonalInfo(ctx *gin.Context) {
	userID, err := getAuthUserID(ctx)
	if err != nil {
		respondError(ctx, http.StatusUnauthorized, err)
		return
	...
	// Update name fields based on what's provided
	if req.FirstNameTH != "" && req.LastNameTH != "" {
		if !isThaiText(req.FirstNameTH) || !isThaiText(req.LastNameTH) {
			respondError(ctx, http.StatusBadRequest, errors.New("Thai name fields must contain Thai characters only"))
			return
		}
		user.FirstNameTH = strings.TrimSpace(req.FirstNameTH)
		user.LastNameTH = strings.TrimSpace(req.LastNameTH)
		user.FirstNameEN = ""
		user.LastNameEN = ""
	} else if req.FirstNameEN != "" && req.LastNameEN != "" {
		if !isEnglishText(req.FirstNameEN) || !isEnglishText(req.LastNameEN) {
			respondError(ctx, http.StatusBadRequest, errors.New("English name fields must contain English characters only"))
			return
		}
		user.FirstNameEN = strings.TrimSpace(req.FirstNameEN)
		user.LastNameEN = strings.TrimSpace(req.LastNameEN)
		user.FirstNameTH = ""
		user.LastNameTH = ""
	}
	...
	if req.Phone != "" {
		...
		if !phoneRegex.MatchString(normalized) {
			respondError(ctx, http.StatusBadRequest, errors.New("invalid phone format (must be 9-10 digits)"))
			return
		}
	...
	if req.Birthday != "" {
		...
		if err != nil {
			respondError(ctx, http.StatusBadRequest, errors.New("invalid birthday format (use YYYY-MM-DD)"))
			return
		}
...
// ApproveStudentProfile - Teacher approves student profile
func (pc *ProfileController) ApproveStudentProfile(ctx *gin.Context) {
	requesterID, err := getAuthUserID(ctx)
	if err != nil {
		respondError(ctx, http.StatusUnauthorized, err)
		return
	...
	// AccountTypeID 2 = Teacher
	if requester.AccountTypeID != 2 {
		respondError(ctx, http.StatusForbidden, errors.New("only teachers can approve student profiles"))
		return
	}
	...
	if err != nil {
		respondError(ctx, http.StatusBadRequest, errors.New("invalid student id"))
		return
	}

	// Find the student
	var student entity.User
	if err := pc.DB.First(&student, studentID).Error; err != nil {
		handleDBError(ctx, err, "student not found")
		return
	}

	// Check if the user is a student (AccountTypeID 1)
	if student.AccountTypeID != 1 {
		respondError(ctx, http.StatusBadRequest, errors.New("can only approve student profiles"))
		return
	}

	// Update profile_completed status
	student.ProfileCompleted = true
	if err := pc.DB.Save(&student).Error; err != nil {
		respondError(ctx, http.StatusInternalServerError, err)
		return
	}
...
// RevokeStudentProfileApproval - Teacher revokes student profile approval
func (pc *ProfileController) RevokeStudentProfileApproval(ctx *gin.Context) {
	requesterID, err := getAuthUserID(ctx)
	if err != nil {
		respondError(ctx, http.StatusUnauthorized, err)
		return
	...
	if requester.AccountTypeID != 2 {
		respondError(ctx, http.StatusForbidden, errors.New("only teachers can manage student profile approvals"))
		return
	}
	...
	if student.AccountTypeID != 1 {
		respondError(ctx, http.StatusBadRequest, errors.New("can only manage student profiles"))
		return
	}
	...
	ctx.JSON(http.StatusOK, gin.H{
		"message": "Profile approval revoked",
		"data": gin.H{
			"user_id":           student.ID,
			"profile_completed": student.ProfileCompleted,
		},
